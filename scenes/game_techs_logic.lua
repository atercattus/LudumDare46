...
function M.new()
    ...
    function M.processBuyAds(deltaTime)
        ...
        for i = #toDelete, 1, -1 do
            ...
            local tech = M.techs[idx]

            if tech.removeCb ~= nil then
                tech.removeCb(tech)
            end

            utils.tableRemove(M.techs, idx)
            tech:removeSelf()
            tech.txt:removeSelf()
        end
    end
    ...
    function M.applyDamage(tech, cnt)
        ...
        for i, _ in next, M.techs do
            if M.techs[i] == tech then
                utils.tableRemove(M.techs, i)
                break
            end
        end
    ...
    function M.getMinTargetY(tech)
        ...
        for _, otherTech in next, M.techs do
